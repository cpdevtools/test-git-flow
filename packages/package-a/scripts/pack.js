...
const projectName = process.env.PROJECT_NAME || '@cpdevtools/test-package-a';
const version = process.env.PROJECT_VERSION || '1.0.0';
const outputDir = process.env.ARTIFACT_OUTPUT_DIR || '.artifacts';
const artifactFilename = process.env.ARTIFACT_FILENAME || 'cpdevtools-test-package-a';

// Simulate creating a package tarball
const tarballName = `cpdevtools-test-package-a-${version}.tgz`;
const tarballPath = join(outputDir, tarballName);

// Create output directory
mkdirSync(outputDir, { recursive: true });

// Simulate tarball creation (in real scenario, npm pack would do this)
